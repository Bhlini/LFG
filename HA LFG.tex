...
Das Bezugswort des Partizips in den Kasus Nominativ, (Genitiv), Akkusativ und Dativ ist in jedem Fall eine grammatikalische Funktion der übergeordneten Struktur, und somit Element des Hauptsatzes:\footnote{Vgl. KSt S. 771, § 138,5a.}\\
($\uparrow$SUBJ) = ((GF$\uparrow$)GF) \\

Die grammatikalische Funktion einer Partizipialkonstruktion ist immer entweder ein XADJ, ein ADJ oder ein XCOMP: \\
($\uparrow$GF) = ((XADJ$\uparrow$)GF) $\mid$ ((ADJ$\uparrow$)GF) $\mid$ ((XCOMP$\uparrow$)GF) \textbf{???} \\

Für ein Bezugswort im Ablativ kann keine derartige Aussage getroffen werden, da es sowohl innerhalb einer PC- als auch einer Abl.-abs.-Konstruktion vorkommen kann, und in letzterem Falle eben gerade keine grammatikalische Funktion der übergeordneten Struktur wäre. \\

--> vielleicht sowas wie "Wenn das Partizip Element eines ADJ ist, steht es immer im Ablativ"?
($\uparrow$ADJ CASE) = abl \textbf{???} \\
(ich denk da wir diese Regeln ja immer im Zusammenhang mit Partizipien formulieren, müssen wir nich überall noch ,,part'' einfügen, das ist ja im Endeffekt eh ne Implementierungssache)

SUBST PART:
\textbf{Variante 1: XADJ}:\\
Falls das Subjekt fehlt, wie beim substantivierten Partizip, ist es ein XADJ zur übergeordneten grammatikalischen Funktion, welche ebenfalls fehlt): \\
($\uparrow$SUBJ\textsubscript{mis}) = ((GF\textsubscript{mis}$\uparrow$)XADJ) \textbf{???} \\

ABL ABS:
Da der Restsatz (S\textsubscript{fin}) auch ohne den Abl. abs. noch Sinn ergeben würde,  steht S\textsubscript{part} in der Funktion eines ADJ: \\
...
--> Wenn das Partizip ein XCOMP der übergeordneten grammatikalischen Funktion ist, ist es meist ein PPA, selten ein PPP: \\
%($\uparrow$XCOMP RELTENSE) = present   \\
$\neg$ ($\uparrow$(RELTENSE)XCOMP) = future \\
