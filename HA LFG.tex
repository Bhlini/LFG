...
\textbf{missum}: & $[1]$ \:  ($\uparrow$PRED) & = & `mitto$\langle$SUBJ, OBL\textsubscript{GOAL}$\rangle$'\\
$\qquad$ & $[2]$ \:  ($\uparrow$SUBJ NUM) & = & sg \\
$\qquad$ & $[2.1]$ \:  \{(($\uparrow$ SUBJ GEN) & = & m \\ 
$\qquad$ & $[2.2]$ \:  ($\uparrow$SUBJ CASE) & = & acc ) $\mid$\\
$\qquad$ & $[2.3]$ \: (($\uparrow$SUBJ GEN) & = & n \\
$\qquad$ & $[2.4]$ \:  ($\uparrow$SUBJ CASE) & = & \{nom $\mid$ acc\} ) \}\\
