...
$\qquad$ & $\qquad$ & $\qquad$ & $\qquad$ \:  (SUBJ, OBJ, OBL\textsubscript{PURPOSE}) $\rangle$'\footnote{Die Aufgeführten Möglichkeiten entsprechen in obiger Reihenfolge den Beispielsätzen \textit{petere auxilium ab aliquo}; \textit{petere hostes in Gallia}; und \textit{petere ab aliquo ut Donaldum Trumpum occidat} \textbf{(Vgl. RHH §119 und § 234)}.} \\  
$\qquad$ & $[2]$ \:  ($\uparrow$SUBJ) & = & ((XADJ$\uparrow$)OBJ)\\
$\qquad$ & $[3]$ \:  ($\uparrow$MOOD) & = & part\\
$\qquad$ & $[4]$ \:  ($\uparrow$PASSIVE) & = & + \\
$\qquad$ & $[5]$ \:  ($\uparrow$RELTENSE) & = & past \\
