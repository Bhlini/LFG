...
Dies ist hier jedoch nur exemplarisch dargestellt, da die erschöpfende Notation aller möglichen grammatikalischen Funktionen für jeden Konstituenten den Rahmen dieser Arbeit bei Weitem überschreiten würde. So müsste z.B. die XP in den obige Regeln konkret in NP, VP und PP getrennt dargestellt werden, damit gezeigt werden kann, dass eine NP, die direkt von S dominiert wird, andere grammatikalische Funktionen erfüllen kann als eine NP, die von einer anderen NP, VP oder PP dominiert wird. So kann das Subjekt einer Partizipialkonstruktion z.B. in allen Kasus vorkommen, nicht nur im Nominativ.
...
Die grammatikalische Funktion jedes Partizips, welches nicht Teil einer finiten Verbform ist, ist immer entweder ein XADJ, ein ADJ oder ein XCOMP der übergeordneten grammatikalischen Funktion: \\
($\uparrow$GF) = ($\uparrow$XADJ) $\mid$ ($\uparrow$ADJ) $\mid$ ($\uparrow$XCOMP) \textbf{???} \\ 
%($\uparrow$GF) = ($\uparrow$XADJ) $\mid$ ((ADJ$\uparrow$)GF) $\mid$ ((XCOMP$\uparrow$)GF) \textbf{???} \\ 
*Pfeile davor oder dahinter?! inside-out?! :( \\
